...
import { SectorRotationService, SectorRotationResult } from './sector-rotation.service';
import * as moment from 'moment';

@Component({
  selector: 'app-sector-rotation',
  templateUrl: './sector-rotation.component.ts',
  styleUrls: ['./sector-rotation.component.css']
})
export class SectorRotationComponent implements OnInit, AfterViewInit {
  @ViewChild('rrgCanvas', { static: false }) rrgCanvas!: ElementRef<HTMLCanvasElement>;
  ...
  public loadSectorData(): void {
    ...
    this.rotationService.getSectorRotation(this.selectedDate, this.lookbackDays).subscribe({
      next: (data) => {
  ...
  public filterData(): void {
    if (this.selectedCategory === 'ALL') {
      ...
      this.filteredData = this.rotationData.filter(item => item.category === this.selectedCategory);
    }
  ...
  private drawRrgGraph(): void {
    ...
    ctx.fillStyle = '#2e7d32'; ctx.fillText('LEADING (+/+)', width - 130, 30);
    ctx.fillStyle = '#f57f17'; ctx.fillText('WEAKENING (+/-)', width - 140, height - 20);
    ctx.fillStyle = '#c62828'; ctx.fillText('LAGGING (-/-)', 20, height - 20);
    ctx.fillStyle = '#1565c0'; ctx.fillText('IMPROVING (-/+)', 20, 30);

    // 4. Plot Sector Coordinates
    // Dynamic scale bounds based on deviations from 100 axis center
    const padding = 60;
    const maxDev = Math.max(...this.filteredData.map(d => Math.max(Math.abs(d.rsRatio - 100), Math.abs(d.rsMomentum - 100))), 1.5);
