  ...
  ngOnInit(): void {
    ...
    this.multibuttonOptions = [
      ...
      },
      {
        ...
        command: async () => {
          await this.priceTargetService.checkProfitTarget(this.autopilotService.currentHoldings);
        }
      },
      {
      ...
      },
      {
        label: 'Test backtest',
        command: async () => {
          const nvo = await this.strategyBuilderService.getBacktestData('NVO', true);
          console.log(nvo.signals.filter((n) => n.recommendation.mfi === 'Bullish'));
        }
      },
      {
      ...
      },
      {
        ...
        command: async () => {
          ...
          const vxx = await this.strategyBuilderService.getBacktestData('VXX', true);
          console.log(vxx);
        }
      },
      {
    ...
    if (backtestResults && (backtestResults.recommendation === 'STRONGSELL' || backtestResults.recommendation === 'SELL')) {
      ...
      if (lastProfitLoss && lastProfitLoss.profit) {
        ...
        if (profit > 0) {
          ...
          const metTarget = await this.priceTargetService.checkProfitTarget(this.autopilotService.currentHoldings, 0);
          if (!metTarget) {
            this.increaseRiskTolerance();
          }
        }
